...
def notify_test_alert(triggered_by: str | None = None) -> bool:
    title = 'TraceNet Test Alert'
    actor = triggered_by or 'Admin'
    body = f'Test alert triggered by {actor}'
    data = {
        'type': 'test_alert',
        'triggered_by': actor
    }
    sent = _send_notification(title, body, data)
    if sent:
        _record_alert('test_alert', actor, body)
    return sent
