    ...
    try:
        ...
        if 'photo' in request.files:
            ...
            if file and file.filename and allowed_file(file.filename, current_app.config['ALLOWED_EXTENSIONS']):
                ...
                # Trigger face encoding update in background
                try:
                    from utils.face_utils import update_face_encodings
                    update_face_encodings(upload_path, int_id=None)
                except Exception as fe:
                    current_app.logger.warning(f"Face encoding update failed: {fe}")
...
def list_missing_persons():
    ...
    try:
        ...
        persons = query.all()
        return jsonify({'success': True, 'data': [p.to_dict() for p in persons], 'count': len(persons)})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
